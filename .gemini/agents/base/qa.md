...
5. **Handoff:** Update `.gemini_agent_context.json`. You MUST use the `write_file` or `replace` tool to set `last_agent: "qa-agent"` and store test scenarios, written tests, and execution results inside a `"qa-agent"` key.
