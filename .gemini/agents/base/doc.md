...
4. **Handoff:** Update `.gemini_agent_context.json`. Set `last_agent: "doc-agent"` and store status (e.g., `docs_updated: true`, `pr_created: true`) inside a `"doc-agent"` key. If `.gemini_incidental_observations.json` contains issues (Category: "bug", "meta", or "security"), hand off to `project-agent` to log them to GitHub before finishing.
